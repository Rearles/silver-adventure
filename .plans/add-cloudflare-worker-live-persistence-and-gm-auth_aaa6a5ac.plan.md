...
**Gap discovered mid-implementation — `TimelineDataService` not covered:** this plan's todos only ever named `TreeDataService` (people). The app has a second, structurally near-identical service, `TimelineDataService` (frontend/src/app/services/timeline-data.service.ts), which loads `data/{world}-events.json` the same static-file way `TreeDataService` used to load people — it was never updated. Deliberately left alone rather than expanding this already-large plan further: after this plan lands, people data is live and events data is still the old static-file/git path, an intentionally inconsistent-but-not-broken interim state. `GET /api/world/:world/events` and `POST /api/world/:world/events` already exist server-side (todos 4 and 6 built both people and events routes), so wiring `TimelineDataService` the same way `TreeDataService` was wired in this plan is a small, well-understood follow-up — worth its own short plan rather than folding in here after the fact.
